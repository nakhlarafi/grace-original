...
NlLen_map = {"Time":3000, "Math":4500, "Lang":280, "Chart": 1000, "Mockito":650, "unknown":2200}
CodeLen_map = {"Time":1000, "Math":2700, "Lang":300, "Chart":5250, "Mockito":1176, "unknown":2800}
args = dotdict({
        ...
        for dBatch in tqdm(train_set.Get_Train(args.batch_size)):
            if index == 0:
                ...
                for k, devBatch in tqdm(enumerate(val_set.Get_Train(len(val_set)))):
                        ...
                        with torch.no_grad():
                            l, pre, _ = model(devBatch[0], devBatch[1], devBatch[2], devBatch[3], devBatch[4], devBatch[5], devBatch[6])
