...
singlenums = {'Time':5, 'Math':2, "Lang":1, "Chart":3, "Mockito":4, "Closure":1, "Cli":4}
...
for i in tqdm(range(int(len(lst) / totalnum) + 1)):
    ...
    for j in range(totalnum):
        ...
        p = subprocess.Popen("CUDA_VISIBLE_DEVICES="+str(4) + " python3 run.py %d %s %f %d %d"%(lst[totalnum * i + j], project, lr, seed, batch_size), shell=True)
        jobs.append(p)
        time.sleep(10)
    for p in jobs:
        p.wait()
