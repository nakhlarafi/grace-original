...
singlenums = {'Time':5, 'Math':2, "Lang":1, "Chart":3, "Mockito":1, "Closure":1}
...
for i in tqdm(range(int(len(lst) / totalnum) + 1)):
    ...
    for j in range(totalnum):
        ...
        p = subprocess.Popen("CUDA_VISIBLE_DEVICES=0,1,2" + " torchrun --nnodes=1 --nproc_per_node=3 --rdzv_id=100 --rdzv_backend=c10d --rdzv_endpoint=virya4:29407 run.py %d %s %f %d %d"%(lst[totalnum * i + j], project, lr, seed, batch_size), shell=True)
